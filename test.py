...
class Benchmark:
        ...
        while not stop_event.is_set():
            if proc.is_running():
                try:
                    ...
                    if self.use_gpu:
                        gpu_resource_usg = Benchmark.monitor_gpu_usage(
    ...
    def __call__(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if self.use_gpu:
                print(
                    f"[{func.__name__}] GPU utilisation max: {stats.get('gpu_utilisation_max', 0)}%"
                )
                print(
                    f"[{func.__name__}] GPU utilisation avg: {stats.get('gpu_utilisation_avg', 0)}%"
                )
                print(
                    f"[{func.__name__}] VRAM usage avg: {stats.get('vram_usage_avg', 0):.2f} MB"
                )
                print(
                    f"[{func.__name__}] VRAM usage max: {stats.get('vram_usage_max', 0):.2f} MB"
                )
