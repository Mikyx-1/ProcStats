...
struct MonitorStats {
    ...
    int num_cores = 0;  // Add number of cores to stats
};
...
std::set<pid_t> get_all_descendants(pid_t root_pid) {
    ...
    while (!to_check.empty()) {
        ...
        while ((entry = readdir(proc_dir)) != nullptr) {
            ...
            // Note: comm can contain spaces and parentheses, so we need to be careful
            ...
            // We need to find the closing parenthesis of comm and then ppid is the next field
            if (tokens.size() >= 4) {
                // Find the position after the comm field (which ends with ')')
                size_t ppid_index = 3; // Default assumption
                
                // Look for the token that ends with ')' to find where comm ends
                for (size_t i = 1; i < tokens.size(); ++i) {
                    if (tokens[i].back() == ')') {
                        ppid_index = i + 2; // state is next, then ppid
                        break;
                    }
                }
            ...
            // If this process is a child of current_pid, add it to check list
            if (ppid == current_pid) {
...
ProcessInfo get_process_info(pid_t pid) {
    ...
    if (!stat_file.is_open()) {
        return info;  // Process may have terminated
    }
    ...
    // Parse the stat file more carefully
    if (tokens.size() >= 15) {
        // Find the closing parenthesis of comm
        size_t base_index = 0;
        for (size_t i = 1; i < tokens.size(); ++i) {
            if (tokens[i].back() == ')') {
                base_index = i + 1; // Points to state field
                break;
            }
        }
        
        if (base_index + 12 < tokens.size()) { // Need at least 13 more fields after state
            try {
                info.utime = std::stol(tokens[base_index + 11]); // utime (14th field overall)
                info.stime = std::stol(tokens[base_index + 12]); // stime (15th field overall)
            } catch (const std::exception&) {
                return info;
            }
        }
    ...
    // Read RAM usage from /proc/[pid]/status
        ...
        while (std::getline(status_file, status_line)) {
            if (status_line.find("VmRSS:") == 0) {
                ...
                if (status_iss >> key >> value_kb >> unit) {
                    info.ram_mb = value_kb / 1024.0;  // Convert KB to MB
                }
                break;
            }
        }
...
// Get aggregated info for all processes in the tree
...
// Calculate CPU usage percentage for process tree (scaled to number of cores)
double calculate_cpu_percentage(const ProcessInfo& prev, const ProcessInfo& curr, 
    ...
    // CPU percentage = (process time delta / system time delta) * 100 * num_cores
    // This scales the result to show total CPU usage across all cores
    // e.g., if using 2 full cores out of 4, this will show 200% instead of 50%
...
MonitorStats monitor_process_tree(pid_t root_pid, double interval_sec, double timeout_sec) {
    ...
    MonitorStats stats;
    stats.num_cores = num_cores;  // Store number of cores in stats
    
    // Initial process tree discovery
    ...
    while (true) {
        ...
        // Record RAM usage (sum of all processes)
...
py::dict full_resource_monitor_fixed(py::function py_func, py::object gpu_index = py::none(),
    ...
    if (pid == 0) {
        // Child process
        try {
            py_func();  // Run Python function
            _exit(0);   // Success
        } catch (...) {
            _exit(1);   // Error in Python function
        }
    }
    
    // Parent process - monitor the child and all its descendants
    ...
    result["num_cores"] = stats.num_cores;  // Include number of cores in results
    
    return result;
}

PYBIND11_MODULE(procstats, m) {
    m.doc() = "Process resource monitoring with child process tracking";
    ...
    m.def("full_resource_monitor", &full_resource_monitor_fixed,
          ...
          py::arg("monitor") = "both",
          py::arg("interval") = 0.1,
          "Monitor CPU and RAM usage including all child processes");
}